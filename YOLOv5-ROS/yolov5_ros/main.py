...
class yolov5_ros(Node):
    def __init__(self):
        ...
        self.pub_fire_position = self.create_publisher(String, 'yolov5/fire_position', 10)  # fire_position 퍼블리셔 추가
        ...
        self.declare_parameter('weights', str(ROOT) + '/config/yolov5s_best.pt')
        self.declare_parameter('data', str(ROOT) + '/data/fire.yaml')
    ...
    def yolovFive2bboxes_msgs(self, bboxes: list, scores: list, cls: list, img_header: Header):
        ...
        for score in scores:
            ...
            # 중심 좌표 계산
            center_x, center_y = self.calculate_center(one_box.xmin, one_box.ymin, one_box.xmax, one_box.ymax)
            one_box.center_x = center_x  # 중심 좌표 x 추가
            one_box.center_y = center_y  # 중심 좌표 y 추가
    ...
    def image_callback(self, image: CompressedImage):
        ...
        msg = self.yolovFive2bboxes_msgs(bboxes=[x_min_list, y_min_list, x_max_list, y_min_list], scores=confidence_list, cls=class_list, img_header=image.header)
        self.pub_bbox.publish(msg)

        self.pub_image.publish(self.bridge.cv2_to_imgmsg(image_raw, "bgr8"))

        if 'fire' in class_list:
            index = class_list.index('fire')
            center_x, _ = self.calculate_center(x_min_list[index], y_min_list[index], x_max_list[index], y_max_list[index])
            fire_position = self.determine_fire_position(center_x, image_raw.shape[1])
            self.pub_fire_position.publish(String(data=fire_position))
        else:
            # 불이 감지되지 않았을 때 no_fire 메시지를 발행
            self.pub_fire_position.publish(String(data='no_fire'))
